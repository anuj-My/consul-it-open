...
export function TaskRow({ task, variant = "detailed", sessionId, isLocked = false }: TaskRowProps) {
    ...
    >
      <div className={`flex items-center gap-4 w-full ${isLocked ? "blur-[2.5px] opacity-50 select-none pointer-events-none" : ""}`}>
        ...
        <div className="shrink-0 cursor-pointer">
          {isCompleted ? (
            <div className="w-6 h-6 rounded bg-teal-400 flex items-center justify-center" onClick={() => completeTask(sessionId, task.id) }>
              <CheckCircle2 className="w-4 h-4 text-white" />
            </div>
          ) : (
            <div className="w-6 h-6 rounded border-2 border-gray-300 bg-white" onClick={() => completeTask(sessionId, task.id) } />
          )}
        </div>
        ...
        <div className="shrink-0 pl-2">
          {task.score ? (
            ...
            <button onClick={() => startTask(sessionId, task.id) }  className="px-5 py-2 rounded-full border border-teal-200 bg-white text-teal-600 text-xs font-semibold hover:bg-teal-50/50 transition-colors">
              Continue
            </button>
          ) : (
            <button onClick={() => startTask(sessionId, task.id) } className="px-6 py-2 rounded-full border border-gray-200 bg-white text-gray-400 text-xs font-bold">
              Start
            </button>
          )}
        </div>
